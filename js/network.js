...
const TransmissionNetwork = {
    ...
    // Physical cross-links are part of the network but are not dedicated paths.
    // Packet/message switching may use these links; circuit switching reserves only one selected route.
    crossLinks: [
        ["A1","B1"],["A2","B2"],["A3","B3"],["A4","B4"],["A5","B5"],
        ["B1","C1"],["B2","C2"],["B3","C3"],["B4","C4"],["B5","C5"],
        ["A2","B3"],["B3","C4"],["A4","B3"],["B2","C3"]
    ],
...
(function injectCleanTransmissionStyles(){
    ...
    style.textContent=`
        ...
        .cleanTransmissionNode.storePoint{border-color:#fb923c!important;box-shadow:0 0 0 4px rgba(251,146,60,.16),0 0 28px rgba(251,146,60,.8)!important}
        .cleanTransmissionNode.circuitReserved{border-color:#facc15!important;box-shadow:0 0 0 3px rgba(250,204,21,.18),0 0 24px rgba(250,204,21,.65)!important}
        .crossNetworkLink{stroke:#94a3b8;stroke-opacity:.32;stroke-width:2.5;stroke-dasharray:7 6;transition:.25s}
        .crossNetworkLink.activeCross{stroke:#f59e0b;stroke-opacity:.9;stroke-width:5;stroke-dasharray:none;filter:drop-shadow(0 0 5px rgba(245,158,11,.75))}
        .circuitLine{stroke-dasharray:10 7;animation:circuitPulse .65s linear infinite}
        .messagePacket{width:42px;height:42px;font-size:26px;border:2px solid #fb923c}
        .circuitPacket{width:30px;height:30px;font-size:19px;border:2px solid #facc15}
        .storeBadge{position:absolute;z-index:25;transform:translate(-50%,-50%);padding:4px 7px;border-radius:9px;background:#7c2d12;color:#fff;font-size:10px;font-weight:800;pointer-events:none;box-shadow:0 0 14px rgba(251,146,60,.7)}
        @keyframes circuitPulse{to{stroke-dashoffset:-17}}
...
function buildTransmissionNetwork(){
    ...
    const pointById=id=>TransmissionNetwork.nodes.find(n=>n.id===id);
    const addLine=(a,b,opts={})=>{
        const p1=pointById(a), p2=pointById(b);
        if(!p1||!p2) return null;
        const line=document.createElementNS("http://www.w3.org/2000/svg","line");
        line.classList.add(opts.cross?"crossNetworkLink":"cleanRouteLine");
        if(opts.cross){ line.dataset.cross="1"; line.dataset.a=a; line.dataset.b=b; }
        else line.dataset.route=opts.route;
        line.setAttribute("x1",p1.x+"%"); line.setAttribute("y1",p1.y+"%");
        line.setAttribute("x2",p2.x+"%"); line.setAttribute("y2",p2.y+"%");
        line.setAttribute("stroke",opts.color||"#94a3b8");
        line.setAttribute("stroke-width",opts.cross?"2.5":"5");
        line.setAttribute("stroke-opacity",opts.cross?".32":".55");
        line.setAttribute("stroke-linecap","round");
        svg.appendChild(line);
        return line;
    };

    // Draw cross-links first so the three main routes remain visually dominant.
    (TransmissionNetwork.crossLinks||[]).forEach(([a,b])=>addLine(a,b,{cross:true}));

    TransmissionNetwork.routes.forEach(route=>{
        for(let i=0;i<route.nodes.length-1;i++){
            addLine(route.nodes[i],route.nodes[i+1],{route:route.id,color:route.color});
        }
    });
...
function highlightTransmissionPath(index){
    document.querySelectorAll(".cleanRouteLine").forEach(line=>{
        ...
        line.setAttribute("stroke-opacity",active?"1":".14");
        line.setAttribute("stroke-width",active?"8":"4");
    });
    document.querySelectorAll(".crossNetworkLink").forEach(line=>line.classList.remove("activeCross"));
...
function clearTransmissionAnimation(){
    ...
    document.querySelectorAll(".cleanPacket,.storeBadge").forEach(p=>p.remove());
    document.querySelectorAll(".cleanTransmissionNode").forEach(n=>n.classList.remove("packetNode","currentHop","storePoint","circuitReserved"));
    document.querySelectorAll(".crossNetworkLink").forEach(l=>l.classList.remove("activeCross"));
}
...
function clearRouteVisualsExcept(nodePath){
    const wanted=new Set(nodePath);
    document.querySelectorAll(".cleanRouteLine").forEach(line=>{
        const r=TransmissionNetwork.routes[Number(line.dataset.route)];
        const active=r && r.nodes.some(n=>wanted.has(n));
        line.setAttribute("stroke-opacity",active?".75":".12");
        line.setAttribute("stroke-width",active?"6":"3");
    });
    document.querySelectorAll(".crossNetworkLink").forEach(line=>{
        if(wanted.has(line.dataset.a)&&wanted.has(line.dataset.b)) line.classList.add("activeCross");
        else line.classList.remove("activeCross");
    });
}

function runPacketSwitching(){
    const result=document.getElementById("resultPanel");
    if(result) result.innerHTML=`<b>📦 Packet Switching</b><br>Message divided into packets. Each packet may take a different route through the network and cross-links.`;
    const packets=Array.from({length:5},(_,i)=>({path:findRandomNetworkPath(),delay:i*500}));
    let done=0;
    packets.forEach((item,index)=>setTimeout(()=>{
        clearRouteVisualsExcept(item.path);
        animateNodePathPacket("📦",item.path,3000,()=>{
            done++; if(done===packets.length) transmissionCompleted("📦 All packets reached the receiver. Different packets used different available routes.");
        },{routeIndex:0});
    },item.delay));
}
function runCircuitSwitching(){
    ...
    const result=document.getElementById("resultPanel");
    if(result) result.innerHTML=`<b>🔌 Circuit Switching</b><br>Step 1: <strong>Establishing dedicated ${r.name}</strong>... No data is sent until the circuit is reserved.`;
    clearTransmissionAnimation();
    const lines=[...document.querySelectorAll(".cleanRouteLine")];
    lines.forEach(l=>l.setAttribute("stroke-opacity",Number(l.dataset.route)===route?".95":".08"));
    r.nodes.forEach(id=>document.getElementById("tnode-"+id)?.classList.add("circuitReserved"));
    setTimeout(()=>{
        if(result) result.innerHTML=`<b>🔌 Circuit Established!</b><br>Dedicated ${r.name} is reserved. Continuous data can now flow without changing routes.`;
        let done=0;
        for(let i=0;i<4;i++) setTimeout(()=>animateNodePathPacket("📦",r.nodes,2600,()=>{
            done++; if(done===4) transmissionCompleted(`🔌 Circuit switching complete. The dedicated ${r.name} remained reserved for the entire transmission.`);
        },{routeIndex:route,circuit:true}),i*430);
    },1100);
}
function runMessageSwitching(){
    ...
    const path=r.nodes;
    const result=document.getElementById("resultPanel");
    if(result) result.innerHTML=`<b>📨 Message Switching — Store & Forward</b><br>One complete message travels hop-by-hop. At every router it is <strong>stored first</strong>, then forwarded.`;
    clearTransmissionAnimation();
    // Unlike circuit switching, no path is reserved. Other links stay visible and the message pauses at every node.
    document.querySelectorAll(".cleanRouteLine").forEach(line=>line.setAttribute("stroke-opacity",Number(line.dataset.route)===route?".8":".2"));
    animateNodePathPacket("📨",path,5200,()=>transmissionCompleted(`📨 The complete message reached the receiver after being stored and forwarded at each router on ${r.name}.`),{routeIndex:route,message:true});
}
