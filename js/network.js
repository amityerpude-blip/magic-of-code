...
/*==================================================
        REAL NETWORK TRANSMISSION SIMULATOR
        17 NODES / 3 ROUTES
==================================================*/

const TransmissionNetwork = {
    nodes: [
        {id:"S", name:"Sender", icon:"💻", x:6, y:50, type:"endpoint"},
        {id:"A1",name:"Router 1",icon:"🔵",x:18,y:18},
        {id:"A2",name:"Router 2",icon:"🔵",x:30,y:18},
        {id:"A3",name:"Router 3",icon:"🔵",x:42,y:18},
        {id:"A4",name:"Router 4",icon:"🔵",x:54,y:18},
        {id:"A5",name:"Router 5",icon:"🔵",x:66,y:18},
        ...
        {id:"C1",name:"Router 11",icon:"🟢",x:18,y:82},
        {id:"C2",name:"Router 12",icon:"🟢",x:30,y:82},
        {id:"C3",name:"Router 13",icon:"🟢",x:42,y:82},
        {id:"C4",name:"Router 14",icon:"🟢",x:54,y:82},
        {id:"C5",name:"Router 15",icon:"🟢",x:66,y:82},

        {id:"R", name:"Receiver",icon:"🖥️",x:94,y:50,type:"endpoint"}
    ],
...
function initializeTransmission(option){
    ...
    Object.assign(canvas.style,{
        ...
        overflow:"hidden",
        borderRadius:"18px",
        background:"radial-gradient(circle at center,rgba(25,45,80,.95),rgba(5,12,28,.98))",
        border:"2px solid rgba(79,195,247,.35)"
    });

    /* SVG NETWORK CABLE LAYER */
    ...
    svg.classList.add("realNetworkLayer");
    canvas.appendChild(svg);

    TransmissionNetwork.svg = svg;

    /* Draw three independent routes */
    TransmissionNetwork.routes.forEach((route,index)=>{
        drawTransmissionRoute(route,index);
    });

    /* Draw nodes */
    TransmissionNetwork.nodes.forEach(nodeData=>{
        const node = document.createElement("div");
        node.className = "transmissionNetworkNode";
        node.id = "tnode-"+nodeData.id;
        node.dataset.id = nodeData.id;

        const isEndpoint = nodeData.type === "endpoint";

        node.innerHTML = `
            <div class="tnIcon">${nodeData.icon}</div>
            <div class="tnName">${nodeData.name}</div>
        `;
        ...
        Object.assign(node.style,{
            ...
            width:isEndpoint?"105px":"82px",
            minHeight:isEndpoint?"82px":"70px",
            zIndex:"5",
            display:"flex",
            ...
            background:isEndpoint
                ?"linear-gradient(145deg,#164e63,#0f172a)"
                :"linear-gradient(145deg,#1e293b,#111827)",
            border:isEndpoint
                ?"2px solid #38bdf8"
                :"2px solid rgba(148,163,184,.55)",
        ...
        const icon=node.querySelector(".tnIcon");
        const name=node.querySelector(".tnName");

        if(icon) icon.style.fontSize=isEndpoint?"28px":"22px";
        if(name){
            name.style.fontSize=isEndpoint?"12px":"10px";
    ...
    legend.className="transmissionLegend";
    legend.innerHTML=`
        <strong>📡 REAL NETWORK</strong>
        <span>🔵 Path 1</span>
        <span>🟣 Path 2</span>
        <span>🟢 Path 3</span>
        <small>Packets can travel through different routers.</small>
    `;
    ...
    if(result){
        result.innerHTML=`
            ...
            💻 Sender → <b>15 routers across 3 paths</b> → 🖥️ Receiver<br>
            Choose a switching method to watch how data travels.
        `;
    }
}

/*==================================================
        DRAW ONE NETWORK ROUTE
==================================================*/

function drawTransmissionRoute(route,index){

    const svg=TransmissionNetwork.svg;
    const canvas=document.getElementById("transmissionCanvas");
    ...
    const points=route.map(id=>{
        const n=TransmissionNetwork.nodes.find(x=>x.id===id);
        return {x:n.x,y:n.y};
    });
    ...
    for(let i=0;i<points.length-1;i++){
        ...
        line.classList.add("routeLine");
        line.dataset.route=index;
...
function createPacket(icon="📦",routeIndex=0){
    ...
    Object.assign(packet.style,{
        ...
        boxShadow:"0 0 14px rgba(56,189,248,.9)",
        transition:"none"
    });

    packet.dataset.route=routeIndex;
...
/*==================================================
        MOVE PACKET THROUGH REAL ROUTE
==================================================*/

function animatePacket(packet,speed,onComplete,routeIndex=0){
    ...
    const step=Math.max(.006,speed/1000);
...
function highlightRoute(routeIndex){
    ...
    document.querySelectorAll(".routeLine").forEach(line=>{

        line.setAttribute(
            "stroke-opacity",
            line.dataset.route==routeIndex ? "1" : ".18"
        );

        line.setAttribute(
            "stroke-width",
            line.dataset.route==routeIndex ? "8" : "4"
        );
    });

    document.querySelectorAll(".transmissionNetworkNode")
        .forEach(node=>node.style.boxShadow="0 5px 14px rgba(0,0,0,.35)");
...
function packetSwitching(){
    ...
    if(result){
        result.innerHTML=`
            ...
            Five packets are sent independently. Watch them use
            different routes through the network.
        `;
    }

    const routes=[0,1,2,0,1];
    let delivered=0;
    ...
    routes.forEach((routeIndex,i)=>{
        ...
        setTimeout(()=>{
            ...
            const packet=createPacket("📦",routeIndex);
            ...
            animatePacket(
                ...
                ()=>{
                    ...
                    if(delivered===routes.length){
                        document.querySelectorAll(".routeLine")
                            .forEach(line=>{
                                line.setAttribute("stroke-opacity",".55");
                                line.setAttribute("stroke-width","5");
                            });
...
function circuitSwitching(){
    ...
    if(result){
        result.innerHTML=`
            ...
            A dedicated route is established first.<br>
            <b>Path 2:</b> Sender → Router 6 → Router 7 → Router 8
            → Router 9 → Router 10 → Receiver
        `;
    }
...
function messageSwitching(){
    ...
    if(result){
        result.innerHTML=`
            <strong>📨 Message Switching</strong><br>
            The complete message travels from node to node
            through <b>Path 3</b>. Each router forwards the message
            before the next router receives it.
        `;
    }
...
function resetTransmission(){
    ...
    canvas.querySelectorAll(".dataPacket")
    ...
    const module=NetworkEngine.simulator.modules.find(
        module=>module.id==="dataTransmission"
    );

    if(!module) return;

    const option=module.options.find(
        option=>option.id===NetworkEngine.state.transmission
    );
    ...
    if(result){
        result.innerHTML=`
            Network reset.<br>
            💻 Sender → 15 Routers / 3 Paths → 🖥️ Receiver
        `;
    }
}
...
function animatePacket(packet,speed,onComplete){
    ...
    if(!canvas || !sender || !receiver || !packet){
        if(typeof onComplete==="function") onComplete();
        return;
    }
    ...
    const startX =
        senderRect.right - canvasRect.left + 8;

    const endX =
        receiverRect.left - canvasRect.left - 8;
...
function packetSwitching(){
    ...
    if(result){
        result.innerHTML=`
            <strong>📦 Packet Switching</strong><br>
            Data is divided into separate packets.
        `;
    }
...
function messageSwitching(){
    ...
    if(result){
        result.innerHTML=`
            <strong>📨 Message Switching</strong><br>
            The complete message travels as one unit.
        `;
    }
...
function circuitSwitching(){
    ...
    const path=document.getElementById("transmissionPath");
    const result=document.getElementById("resultPanel");

    if(path){
        path.classList.add("activeCircuit");
    }
    ...
    setTimeout(()=>{
        ...
        if(result){
            result.innerHTML=`
                <strong>🔌 Circuit Established</strong><br>
                Data is travelling through the dedicated network.
            `;
        }
            ...
            setTimeout(()=>{
                ...
                animatePacket(packet,6,()=>{
                    ...
                    if(delivered===3){
                        ...
                        if(path){
                            path.classList.remove("activeCircuit");
                        }
...
function resetTransmission(){
    ...
    canvas.querySelectorAll(".dataPacket").forEach(
    ...
    if(result){
        result.innerHTML="Transmission reset. Ready to send.";
    }
}
