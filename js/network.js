...
const TransmissionNetwork = {
    ...
    /* Additional links make the network a real interconnected network.
       They are visible, but the three coloured paths remain the main
       teaching paths used for circuit switching. */
    crossLinks: [
        ["A1","B1"],["A2","B2"],["A3","B3"],["A4","B4"],["A5","B5"],
        ["B1","C1"],["B2","C2"],["B3","C3"],["B4","C4"],["B5","C5"]
    ],

    /* Packet-switching routes deliberately use some cross-links so
       students can see packets taking different node-to-node routes. */
    packetRoutes: [
        ["S","A1","A2","B2","B3","B4","A4","A5","R"],
        ["S","B1","B2","C2","C3","C4","B4","B5","R"],
        ["S","C1","C2","C3","B3","B4","C4","C5","R"],
        ["S","A1","B1","C1","C2","B2","A2","A3","R"]
    ],
...
(function injectCleanTransmissionStyles(){
    ...
    style.textContent=`
        ...
        .cleanTransmissionNode.storeNode{border-color:#fb923c!important;box-shadow:0 0 0 4px rgba(251,146,60,.18),0 0 28px rgba(251,146,60,.75)!important}
        .storeBadge{position:absolute;top:-15px;right:-18px;padding:3px 6px;border-radius:9px;background:#f97316;color:#fff;font-size:8px;font-weight:900;letter-spacing:.4px;box-shadow:0 0 10px rgba(249,115,22,.8);z-index:25;animation:storePulse .55s ease-in-out infinite alternate}
        .circuitReserved{stroke:#facc15!important;stroke-opacity:1!important;stroke-width:9!important;filter:drop-shadow(0 0 5px rgba(250,204,21,.9))}
        .crossLinkLine{stroke:#94a3b8;stroke-width:2.5;stroke-opacity:.42;stroke-dasharray:7 7}
        .crossLinkLine.activePacketLink{stroke:#f59e0b;stroke-width:5;stroke-opacity:.95;stroke-dasharray:5 5;filter:drop-shadow(0 0 4px rgba(245,158,11,.8))}
        .messageFlowLine{stroke:#fb923c!important;stroke-width:6!important;stroke-opacity:.95!important;stroke-dasharray:3 7!important}
        .messageStorePanel{border-color:rgba(251,146,60,.5)!important;background:rgba(67,32,8,.35)!important}
        .circuitFlowPanel{border-color:rgba(250,204,21,.5)!important;background:rgba(63,49,4,.28)!important}
        ...
        @keyframes storePulse{from{transform:scale(.92)}to{transform:scale(1.08)}}
...
function updateTransmissionTechniqueUI(technique){
    ...
    panel.classList.toggle("messageStorePanel",technique==="messageSwitching");
    panel.classList.toggle("circuitFlowPanel",isCircuit);
    panel.querySelector("h3").innerHTML=isCircuit
        ? "🛣️ Select a Dedicated Circuit Path"
        : technique==="messageSwitching"
            ? "📨 Automatic Store-and-Forward Route"
...
function buildTransmissionNetwork(){
    ...
    TransmissionNetwork.routes.forEach(route=>{
        ...
        for(let i=0;i<pts.length-1;i++){
            ...
            line.dataset.from=pts[i].id;
            line.dataset.to=pts[i+1].id;
    ...
    /* Cross-links: these are real network links, not decorative lines. */
    TransmissionNetwork.crossLinks.forEach(link=>{
        const a=TransmissionNetwork.nodes.find(n=>n.id===link[0]);
        const b=TransmissionNetwork.nodes.find(n=>n.id===link[1]);
        if(!a||!b) return;
        const line=document.createElementNS("http://www.w3.org/2000/svg","line");
        line.classList.add("crossLinkLine");
    ...
    legend.innerHTML=`<b>🌐 LIVE NETWORK</b><span>🔵 Path 1</span><span>🟣 Path 2</span><span>🟢 Path 3</span><span>⚪ Cross-links</span><span>17 nodes</span>`;
...
function clearTransmissionAnimation(){
    ...
    document.querySelectorAll(".cleanTransmissionNode").forEach(n=>n.classList.remove("packetNode","currentHop","storeNode"));
    document.querySelectorAll(".crossLinkLine").forEach(l=>l.classList.remove("activePacketLink"));
    document.querySelectorAll(".cleanRouteLine").forEach(l=>l.classList.remove("circuitReserved","messageFlowLine"));
}
...
function markCurrentHop(point,routeIndex,hopIndex,totalHops,mode="packet"){
    ...
    if(result && point){
        ...
        const prefix=mode==="message" ? "📨 Store-and-Forward" : mode==="circuit" ? "🔌 Dedicated Circuit" : "📡 Data is travelling";
        result.innerHTML=`<b>${prefix}</b><br>${route.label} &nbsp;|&nbsp; Hop ${Math.min(hopIndex+1,totalHops)} of ${totalHops}<br><strong>Current node: ${point.name}</strong>`;
    }
}

function showStoreBadge(nodeId){
    const node=document.getElementById("tnode-"+nodeId);
    if(!node) return;
    node.classList.add("storeNode");
    const old=node.querySelector(".storeBadge");
    if(old) old.remove();
    const badge=document.createElement("span");
    badge.className="storeBadge";
    badge.textContent="STORE";
    node.appendChild(badge);
    setTimeout(()=>badge.remove(),700);
}

function highlightPacketRoute(nodes){
    document.querySelectorAll(".crossLinkLine").forEach(line=>line.classList.remove("activePacketLink"));
    document.querySelectorAll(".cleanRouteLine").forEach(line=>line.setAttribute("stroke-opacity",".18"));
    for(let i=0;i<nodes.length-1;i++){
        const a=nodes[i], b=nodes[i+1];
        const main=document.querySelector(`.cleanRouteLine[data-from="${a}"][data-to="${b}"],.cleanRouteLine[data-from="${b}"][data-to="${a}"]`);
        if(main){
            main.setAttribute("stroke-opacity",".95");
            main.setAttribute("stroke-width","7");
        }else{
            const cross=document.querySelector(`.crossLinkLine[data-from="${a}"][data-to="${b}"],.crossLinkLine[data-from="${b}"][data-to="${a}"]`);
            if(cross) cross.classList.add("activePacketLink");
        }
    }
}

function animateCleanPacket(icon,routeIndex,duration,onDone,options={}){
    const canvas=document.getElementById("transmissionCanvas");
    ...
    const segmentDuration=Math.max(420,duration/(points.length-1));
    const mode=options.mode||"packet";
    const storeDelay=options.storeDelay||850;
    let segment=0;
    ...
    function travelSegment(){
        ...
        markCurrentHop(a,routeIndex,segment,points.length-1,mode);
        ...
        function frame(now){
            ...
            if(t<1){
                ...
                markCurrentHop(b,routeIndex,segment+1,points.length-1,mode);
                const arrivedAtRouter=!b.endpoint;
                segment++;
                if(mode==="message" && arrivedAtRouter){
                    showStoreBadge(b.id);
                    const result=document.getElementById("resultPanel");
                    if(result) result.innerHTML=`<b>📨 Message stored at ${b.name}</b><br>Complete message received here. <strong>Store → Process → Forward</strong><br>⏳ Holding before the next hop...`;
                    setTimeout(travelSegment,storeDelay);
                }else{
                    setTimeout(travelSegment,mode==="circuit"?70:180);
                }
            }
        }
        const id=requestAnimationFrame(frame);
...
function runPacketSwitching(){
    const result=document.getElementById("resultPanel");
    if(result) result.innerHTML=`<b>📦 Packet Switching</b><br>Packets are independent. Watch them take different routes through the cross-links.`;

    const packets=[
        {nodes:TransmissionNetwork.packetRoutes[0],delay:0},
        {nodes:TransmissionNetwork.packetRoutes[1],delay:500},
        {nodes:TransmissionNetwork.packetRoutes[2],delay:1000},
        {nodes:TransmissionNetwork.packetRoutes[3],delay:1500},
        {nodes:TransmissionNetwork.routes[randomRouteIndex()].nodes,delay:2000}
    ];

    let done=0;
    packets.forEach(item=>setTimeout(()=>{
        const routeIndex=findClosestTeachingRoute(item.nodes);
        highlightPacketRoute(item.nodes);
        animateRouteNodes("📦",item.nodes,2600,"packet",()=>{
            done++;
            if(done===packets.length){
                document.querySelectorAll(".cleanTransmissionNode").forEach(n=>n.classList.remove("currentHop"));
                transmissionCompleted("📦 All packets reached the receiver. Different packets used different node-to-node routes, including cross-links.");
            }
        });
    },item.delay));
}

function findClosestTeachingRoute(nodes){
    let best=0,bestScore=-1;
    TransmissionNetwork.routes.forEach((route,i)=>{
        const score=route.nodes.filter(id=>nodes.includes(id)).length;
        if(score>bestScore){bestScore=score;best=i;}
    });
    return best;
}

function animateRouteNodes(icon,nodes,duration,mode,onDone){
    const points=nodes.map(id=>TransmissionNetwork.nodes.find(n=>n.id===id)).filter(Boolean);
    const canvas=document.getElementById("transmissionCanvas");
    if(!canvas){onDone?.();return;}
    const packet=document.createElement("div");
    packet.className="cleanPacket";
    packet.textContent=icon;
    canvas.appendChild(packet);
    const segmentDuration=Math.max(350,duration/Math.max(1,points.length-1));
    let segment=0;
    ...
    function travel(){
        ...
        const node=document.getElementById("tnode-"+a.id);
        document.querySelectorAll(".cleanTransmissionNode").forEach(n=>n.classList.remove("currentHop","storeNode"));
        node?.classList.add("currentHop");
        const result=document.getElementById("resultPanel");
        if(result) result.innerHTML=`<b>📦 Packet Switching</b><br>Hop ${segment+1} of ${points.length-1}<br><strong>${a.name} → ${b.name}</strong>`;
        const start=performance.now();
        function frame(now){
            const t=Math.min(1,(now-start)/segmentDuration);
            packet.style.left=(a.x+(b.x-a.x)*t)+"%";
            packet.style.top=(a.y+(b.y-a.y)*t)+"%";
            if(t<1){
                const id=requestAnimationFrame(frame);TransmissionNetwork.animationIds.push(id);
            }else{
                segment++;setTimeout(travel,90);
            }
        }
        const id=requestAnimationFrame(frame);TransmissionNetwork.animationIds.push(id);
    }
...
function runCircuitSwitching(){
    ...
    const result=document.getElementById("resultPanel");
    if(result) result.innerHTML=`<b>🔌 Circuit Switching</b><br>First, a dedicated <strong>${r.name}</strong> is established. The path stays reserved while data is transmitted.`;

    highlightTransmissionPath(route);
    document.querySelectorAll(`.cleanRouteLine[data-route="${route}"]`).forEach(line=>line.classList.add("circuitReserved"));

    setTimeout(()=>{
        let done=0;
        for(let i=0;i<3;i++){
            setTimeout(()=>animateCleanPacket("📦",route,3200,()=>{
                done++;
                if(done===3) transmissionCompleted(`🔌 ${r.name} remained dedicated for the whole transmission. The three data units stayed on the same reserved path.`);
            },{mode:"circuit"}),i*550);
        }
    },1000);
}
...
function runMessageSwitching(){
    ...
    const r=TransmissionNetwork.routes[route];
    const result=document.getElementById("resultPanel");
    if(result) result.innerHTML=`<b>📨 Message Switching</b><br>The complete message travels as ONE unit. At every router it <strong>stops → is stored → then forwarded</strong>.`;

    highlightTransmissionPath(route);
    document.querySelectorAll(`.cleanRouteLine[data-route="${route}"]`).forEach(line=>line.classList.add("messageFlowLine"));
    animateCleanPacket("📨",route,5200,()=>transmissionCompleted(`📨 Complete message delivered. It was stored and forwarded at each intermediate router on ${r.name}.`),{mode:"message",storeDelay:900});
}
