...
const TransmissionNetwork = {
    ...
    // Real network links between the three main routes. These are visible in the
    // network and are also available to packet/message switching. Circuit
    // switching deliberately ignores them once a dedicated path is selected.
    crossLinks: [
    ...
    // Alternate paths used to demonstrate packet/message routing across
    // cross-links instead of making every transmission look like a circuit.
    alternateRoutes: [
        {name:"Cross Route A", nodes:["S","A1","A2","B2","B3","B4","A5","R"]},
        {name:"Cross Route B", nodes:["S","B1","B2","A3","A4","A5","R"]},
        {name:"Cross Route C", nodes:["S","C1","C2","B3","B4","C5","R"]},
        {name:"Cross Route D", nodes:["S","A1","B1","B2","C2","C3","C4","R"]},
        {name:"Cross Route E", nodes:["S","B1","C1","C2","C3","B4","B5","R"]}
    ],
...
(function injectCleanTransmissionStyles(){
    ...
    style.textContent=`
        ...
        .cleanCrossLink{stroke:#94a3b8;stroke-width:2.5;stroke-opacity:.38;stroke-dasharray:7 6;transition:.2s}
        .cleanCrossLink.activeCross{stroke:#facc15;stroke-width:5;stroke-opacity:.95;filter:drop-shadow(0 0 5px rgba(250,204,21,.75))}
        ...
        .cleanTransmissionNode.storingNode{border-color:#fb923c!important;box-shadow:0 0 0 4px rgba(251,146,60,.18),0 0 30px rgba(251,146,60,.85)!important}
        .storeBadge{position:absolute;right:-9px;top:-13px;padding:3px 6px;border-radius:9px;background:#fb923c;color:#111827;font-size:9px;font-weight:900;box-shadow:0 0 12px rgba(251,146,60,.9);z-index:30}
...
function updateTransmissionTechniqueUI(technique){
    ...
    panel.querySelector("h3").innerHTML=isCircuit
        ...
        : (technique==="messageSwitching" ? "📨 Store-and-Forward Routing" : "🎲 Automatic Packet Routing");
...
function buildTransmissionNetwork(){
    ...
    // Draw the actual inter-route links after the main paths so they are clearly
    // visible as part of the topology rather than being mistaken for another path.
    TransmissionNetwork.crossLinks.forEach((pair,index)=>{
        const a=TransmissionNetwork.nodes.find(n=>n.id===pair[0]);
        const b=TransmissionNetwork.nodes.find(n=>n.id===pair[1]);
        if(!a||!b) return;
        const line=document.createElementNS("http://www.w3.org/2000/svg","line");
        line.classList.add("cleanCrossLink");
        line.dataset.cross=index;
...
function getNamedRoutePoints(nodes){
    return nodes.map(id=>TransmissionNetwork.nodes.find(n=>n.id===id)).filter(Boolean);
}

function highlightCrossLinksForNodes(nodes){
...
function animateNodeSequence(icon, points, duration, options={}, onDone){
    const canvas=document.getElementById("transmissionCanvas");
    if(!canvas||!points||points.length<2){onDone?.();return;}
    ...
    function finish(){
        ...
        stopped=true;
        packet.remove();
        document.querySelectorAll(".cleanTransmissionNode").forEach(n=>n.classList.remove("packetNode","currentHop","storingNode"));
        document.querySelectorAll(".storeBadge").forEach(b=>b.remove());
        onDone?.();
    }

    function travelSegment(){
        if(segment>=points.length-1){ finish(); return; }

        const a=points[segment];
        const b=points[segment+1];
        markCurrentHop(a,0,segment,points.length-1);
        ...
        function frame(now){
            ...
            if(t<1){
                ...
                markCurrentHop(b,0,segment+1,points.length-1);
                segment++;
...
function animateCleanPacket(icon,routeIndex,duration,onDone){
    ...
    highlightCrossLinksForNodes(points);
    animateNodeSequence(icon,points,duration,{},onDone);
}

function storeAndForwardPause(node,duration,continueFn){
    const el=document.getElementById("tnode-"+node.id);
    if(el){
        el.classList.add("storingNode");
    ...
    const result=document.getElementById("resultPanel");
    if(result){
        result.innerHTML=`<b>📨 Message Switching — STORE</b><br><strong>${node.name}</strong> has received the complete message and is storing it before forwarding.`;
    }
    setTimeout(()=>{
        if(el){
            el.classList.remove("storingNode");
            el.querySelector(".storeBadge")?.remove();
        }
        if(result){
            result.innerHTML=`<b>📨 Message Switching — FORWARD</b><br><strong>${node.name}</strong> has stored the complete message and is now forwarding it to the next node.`;
        }
        continueFn();
    },duration);
}

function animateMessageSwitching(points,onDone){
    // The packet icon is deliberately one complete envelope, not several packets.
    // At every intermediate router the envelope stops, the router displays STORE,
    // then changes to FORWARD before the envelope moves to the next router.
    highlightCrossLinksForNodes(points);
...
function runPacketSwitching(){
    const result=document.getElementById("resultPanel");
    if(result) result.innerHTML=`<b>📦 Packet Switching</b><br>Five packets are being sent. Different packets may use different routes and cross-links.`;
    ...
    chosen.forEach((item,index)=>setTimeout(()=>{
        highlightTransmissionPath(index%3);
        ...
        animateNodeSequence("📦",points,3200,{},()=>{
            ...
            if(done===chosen.length){
                ...
                transmissionCompleted("📦 All packets reached the receiver. Different packets used different routes, including cross-links.");
            }
        });
    },index*650));
}
...
function runCircuitSwitching(){
    ...
    const result=document.getElementById("resultPanel");
    if(result) result.innerHTML=`<b>🔌 Circuit Switching — ESTABLISHING</b><br>Setting up a dedicated <strong>${r.name}</strong>. No data is sent until the circuit is established.`;

    // A circuit is deliberately one fixed route. Cross-links are dimmed rather
    // than used so students can see the difference from packet/message switching.
    highlightTransmissionPath(route);
    document.querySelectorAll(".cleanCrossLink").forEach(line=>line.classList.remove("activeCross"));

    setTimeout(()=>{
        if(result) result.innerHTML=`<b>🔌 Circuit Established</b><br><strong>${r.name}</strong> is now reserved. Continuous data is travelling on this one dedicated path.`;
        let done=0;
        for(let i=0;i<3;i++){
            setTimeout(()=>animateCleanPacket("📦",route,3000,()=>{
                done++;
                if(done===3) transmissionCompleted(`🔌 Dedicated ${r.name} stayed reserved for the complete transmission. No cross-link was used.`);
            }),i*550);
        }
    },1200);
}
...
function runMessageSwitching(){
    const candidate=TransmissionNetwork.alternateRoutes[Math.floor(Math.random()*TransmissionNetwork.alternateRoutes.length)];
    const points=getNamedRoutePoints(candidate.nodes);
    const result=document.getElementById("resultPanel");
    if(result) result.innerHTML=`<b>📨 Message Switching</b><br>One complete message is travelling through <strong>${candidate.name}</strong>. Watch each router STORE the message, pause, then FORWARD it.`;
    ...
    highlightCrossLinksForNodes(points);
    animateMessageSwitching(points,()=>transmissionCompleted(`📨 The complete message reached the receiver after STORE → WAIT → FORWARD at each intermediate router on ${candidate.name}.`));
}
