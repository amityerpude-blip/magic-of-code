...
function renderTransmissionModule(module){
    ...
    section.innerHTML=`
        <div class="moduleHeader">
            ...
            <p>${module?.description||"Choose a transmission method, select a network path, and watch data travel through routers."}</p>
        </div>

        <div class="transmissionTechniqueGrid" id="transmissionTechniqueGrid"></div>

        <div class="transmissionPathPanel">
            <h3>🛣️ Select a Network Path</h3>
    ...
    techniques.forEach(t=>{
        ...
        btn.addEventListener("click",()=>{
            ...
            updateTransmissionStatus(`Selected <b>${t.title}</b>. Now choose a path and click <b>Transmit Data</b>.`);
    ...
    TransmissionNetwork.routes.forEach(route=>{
        ...
        btn.addEventListener("click",()=>selectTransmissionPath(route.id));
...
function buildTransmissionNetwork(){
    ...
    TransmissionNetwork.routes.forEach(route=>{
        ...
        for(let i=0;i<pts.length-1;i++){
            ...
            line.setAttribute("stroke",route.color); line.setAttribute("stroke-width","5"); line.setAttribute("stroke-opacity",".55"); line.setAttribute("stroke-linecap","round");
            svg.appendChild(line);
        }
    });
    ...
    legend.innerHTML=`<b>REAL NETWORK</b><span>🔵 Path 1</span><span>🟣 Path 2</span><span>🟢 Path 3</span><span>17 nodes</span>`;
    canvas.appendChild(legend);
    highlightTransmissionPath(TransmissionNetwork.selectedPath);
    const result=document.getElementById("resultPanel");
    if(result) result.innerHTML=`<b>📡 Ready.</b> Select a transmission technique, select a path, then click <b>Transmit Data</b>.`;
}
...
function clearTransmissionAnimation(){
    ...
    document.querySelectorAll(".cleanTransmissionNode").forEach(n=>n.classList.remove("packetNode"));
}
...
function animateCleanPacket(icon,routeIndex,duration,onDone){
    ...
    function frame(now){
        ...
        const scaled=elapsed*(points.length-1);
        ...
        const nearNode=points[Math.round(scaled)];
        document.querySelectorAll(".cleanTransmissionNode").forEach(n=>n.classList.remove("packetNode"));
        if(nearNode) document.getElementById("tnode-"+nearNode.id)?.classList.add("packetNode");
        if(elapsed<1){
...
function runPacketSwitching(){
    const result=document.getElementById("resultPanel");
    if(result) result.innerHTML=`<b>📦 Packet Switching</b><br>Five packets are travelling independently. Watch them take different routes.`;
    const routes=[0,1,2,0,1];
    let done=0;
    routes.forEach((route,i)=>setTimeout(()=>{
        highlightTransmissionPath(route);
        animateCleanPacket("📦",route,2200,()=>{
            done++;
            if(done===routes.length){
                selectTransmissionPath(TransmissionNetwork.selectedPath);
                transmissionCompleted("📦 All packets reached the receiver.");
            }
        });
    },i*450));
}
...
function runCircuitSwitching(){
    ...
    const result=document.getElementById("resultPanel");
    if(result) result.innerHTML=`<b>🔌 Circuit Switching</b><br>Dedicated <b>${r.name}</b> established. All data uses this one route.`;
    highlightTransmissionPath(route);
    setTimeout(()=>{
        let done=0;
        for(let i=0;i<3;i++) setTimeout(()=>animateCleanPacket("📦",route,1800,()=>{
            done++; if(done===3) transmissionCompleted("🔌 Circuit established and data delivered.");
        }),i*350);
    },600);
}
...
function runMessageSwitching(){
    ...
    const r=TransmissionNetwork.routes[route];
    const result=document.getElementById("resultPanel");
    if(result) result.innerHTML=`<b>📨 Message Switching</b><br>The complete message is stored and forwarded through <b>${r.name}</b>.`;
    highlightTransmissionPath(route);
    animateCleanPacket("📨",route,3600,()=>transmissionCompleted("📨 Complete message reached the receiver."));
}
...
function resetTransmission(){
    ...
    selectTransmissionPath(TransmissionNetwork.selectedPath);
    const result=document.getElementById("resultPanel");
    if(result) result.innerHTML=`<b>🔄 Network reset.</b> Select a technique and path, then transmit again.`;
}
