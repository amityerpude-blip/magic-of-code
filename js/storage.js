...
function dailyMissionStatus(){
    ...
    return{
        comics:Math.min(5,mission.comicPages.length),
        quizzes:Math.min(3,mission.quizQuestions),
        coding:mission.codingComplete?1:0,
        complete:mission.comicPages.length>=5&&mission.quizQuestions>=3&&mission.codingComplete,
        rewarded:mission.rewardClaimed
    };
}
...
function recordComicPage(page){
    const mission=getDailyMission();
    const n=Number(page);
    if(Number.isFinite(n)&&n>0&&!mission.comicPages.includes(n)){mission.comicPages.push(n);mission.comicPages.sort((a,b)=>a-b);saveDailyMission(mission);checkDailyMission();}
}
function recordQuizQuestion(){
    const mission=getDailyMission();
    if(mission.quizQuestions<3){mission.quizQuestions++;saveDailyMission(mission);checkDailyMission();}
}
function recordCodingChallenge(){
    const mission=getDailyMission();
    if(!mission.codingComplete){mission.codingComplete=true;saveDailyMission(mission);checkDailyMission();}
}
function missionPopupStyles(){
    ...
    style.textContent=`
    .dailyMissionOverlay{position:fixed;inset:0;display:none;align-items:center;justify-content:center;background:rgba(2,8,23,.72);backdrop-filter:blur(8px);z-index:100000;padding:20px}
    .dailyMissionOverlay.show{display:flex;animation:dailyMissionFade .2s ease-out}
    .dailyMissionBox{width:min(460px,94vw);background:linear-gradient(145deg,#17243a,#0d1729);border:1px solid rgba(255,213,79,.4);border-radius:28px;padding:28px;color:#fff;box-shadow:0 25px 70px rgba(0,0,0,.55);text-align:center}
    .dailyMissionBox h2{color:#ffd54f;margin:0 0 8px;font-size:28px}.dailyMissionBox p{opacity:.9;margin:0 0 20px}.dailyMissionTasks{text-align:left;display:grid;gap:10px}.dailyMissionTask{padding:12px 14px;border-radius:14px;background:rgba(255,255,255,.07);display:flex;justify-content:space-between;gap:12px}.dailyMissionTask.done{background:rgba(46,204,113,.14)}.dailyMissionTask b{color:#ffd54f}.dailyMissionReward{margin-top:18px;padding:13px;border-radius:14px;background:rgba(255,213,79,.1);color:#ffe58a;font-weight:700}.dailyMissionClose{margin-top:20px;border:0;border-radius:999px;padding:12px 28px;background:linear-gradient(135deg,#2ecc71,#ffd54f);font-weight:700;cursor:pointer}
    .dailyMissionComplete .dailyMissionBox{border-color:rgba(46,204,113,.65);box-shadow:0 0 55px rgba(46,204,113,.22),0 25px 70px rgba(0,0,0,.55)}
    @keyframes dailyMissionFade{from{opacity:0;transform:scale(.96)}to{opacity:1;transform:scale(1)}}`;
    document.head.appendChild(style);
}
function ensureDailyMissionOverlay(){
    missionPopupStyles();
    let overlay=document.getElementById("dailyMissionOverlay");
    if(overlay)return overlay;
    overlay=document.createElement("div");overlay.id="dailyMissionOverlay";overlay.className="dailyMissionOverlay";document.body.appendChild(overlay);return overlay;
}
