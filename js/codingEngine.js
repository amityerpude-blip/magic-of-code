    ...
    if(Array.isArray(data.packages) && data.packages.length){
        for(const pkg of data.packages){
            try{
                ...
                console.warn("Package loading failed", pkg, error);
            }
        }
    }
...
async function runPythonCode(){
    ...
    try{
        ...
        // Connect Python input() with browser popup
...
function loadChallenge(id){
    const challenge = currentKingdom.coding.challenges.find(c=>c.id===id);
    if(!challenge)return;

    document.getElementById("codeEditor").value = challenge.code;
    showOutput("📜 " + challenge.description);
}

function resetChallenge(){
    if(currentKingdom.coding.defaultCode){
        document.getElementById("codeEditor").value = currentKingdom.coding.defaultCode;
    }
    showOutput("✨ Challenge Reset.");
}

function copyCode(){
    navigator.clipboard.writeText(document.getElementById("codeEditor").value);
    showOutput("📋 Code copied.");
}
