...
return `<footer class="kingdomFooter"><h3>🎉 Congratulations!</h3><p>You have completed <b>${data.title}</b> Continue your magical journey.</p><button id="nextKingdom">${data.footer.button}</button></footer>`;
}
...
/*
   IMPORTANT:
   Spider Web Nexus owns its network shell through
   spider-web-components.js. Do not let the shared component
   engine create it. All other kingdoms keep the old behaviour.
*/
try{
    if(data.id === "network"){
...
if(completeBtn){
    completeBtn.onclick=()=>{
        ...
        if(data.footer){
            document.getElementById("kingdomContainer").insertAdjacentHTML("beforeend",FooterComponent(data));
        }
    };
}
